...
#def drawtest(s, r, nclr):
#
#    s.fill((0, 0, 0, 255))
#    pygame
#    s2 = s.copy()

def drawoldgradientcircle(s, r, nclr):
...
def makenewnodeclass(nodetype):
    class NewNode(Node):
        ...
        def redraw(n):
            n._img = Surface([2*n.mass, 2*n.mass], HWSURFACE | SRCALPHA, 32)
            n._img.fill([0, 0, 0, 0])
            try:
                mask = n.mask
            except:
                n.mask = None
            if n.mask == None or n.mask.get_size()[0] != 2*n.mass:
                print "new mask for", n.name
                n.mask = createmask(n._img, n.mass)
            if n.ntype == T_PIC:
                n._img = drawimgpart(n, n._img, n.mass, n.mask)
            elif n.ntype == T_CIRC:
                n.img = drawgradientcircle(n._img, n.mass, n.clr, n.mask)
...
class PlanetGraph(Graph):
    ...
    def newNode(g, name):
        ...
        n.clr = random.choice(colordict.THECOLORS.values())
        x, y = random.randint(0, w), random.randint(0, h)
        n.cpos = NumList([x, y])
        #n.speed = NumList([random.randint(0, 10), random.randint(0, 10)])
        n.speed = NumList([0, 0])
        n.mass = Options.maxmass # random.randint(5, Options.maxmass)
        n.redraw()
        ...
        for j in range(len(g.nodes)):
            ...
            e.gravforce = random.uniform(1, 2)
...
class Options(object):
    ...
    maxmass = 30
    nodes = 5
    friction = 0.999 # new speed pr step
    collision = 'none'
    ...
    while True:
        ...
        for edge in g.edges:
            ...
            if dvl > 0:
                ...
                # normalize force direction vector
                # dv = dv / dvl
                # normalize force magnitude
                # norm_dvl = dvl / maxsize
                # apply force to direction vector
